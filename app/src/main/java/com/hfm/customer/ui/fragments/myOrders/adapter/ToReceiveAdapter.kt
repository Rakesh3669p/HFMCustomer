...
class ToReceiveAdapter @Inject constructor() :
    RecyclerView.Adapter<ToReceiveAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val bind: ItemBulkOrderBinding) :
        ...
        fun bind(data: String) {
            with(bind) {
                ...
                requestStatus.text = "Estimated delivery on May 02"
                val orangeColor = ContextCompat.getColor(context, R.color.green)
...
//                requestedDate.text = "${data.date_needed} | ${data.request_time}"
    ...
    override fun onBindViewHolder(holder: ToReceiveAdapter.ViewHolder, position: Int) {
        holder.bind("")
    }
